...
def create_ljspeech_hparams(hparams_string=None, verbose=False):
    """Create model hyperparameters. Parse nondefault from given string."""
...
#         distributed_run=False,
        ...
        ################################
        load_mel_from_disk=False,
        training_files='LJSpeech-1.1/train_metadata.csv',
        validation_files='LJSpeech-1.1/valid_metadata.csv',
        ...
        ################################
        max_wav_value=32768.0,
        sampling_rate=22050, # ljspeech sampling rate
        filter_length=1024,
        hop_length=256, # number audio of frames between stft colmns, default win_length/4
        win_length=1024, # win_length int <= n_ftt: fft window size (frequency domain), defaults to win_length = n_fft
        n_mel_channels=80,
        ...
        ################################
        use_saved_learning_rate=False,
#         learning_rate=1e-3, # changed by smksyj
        learning_rate=1e-4,
        weight_decay=1e-6,
        grad_clip_thresh=1.0,
        batch_size=64,
